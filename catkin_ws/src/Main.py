...
parser.add_argument("--world", type=str, default="/home/ds@123/Yash-DroneStark/yash_nodejs_support/MobileRobot/Debug/Repo/yash_new_qr_map.json")
...
with open('/home/ds@123/Yash-DroneStark/yash_nodejs_support/MobileRobot/Debug/Repo/ds_node_table.csv', 'r') as read_obj:
...
def checkBlueColor(frame):
    ...
    lower_thresh_blue = np.array([72,136,136])
    upper_thresh_blue = np.array([179,255,224])
    ...
    if(len(contours) != 0):
        ...
        if img is not None:
            ...
            for i in range(len(deltaX)-1):
                # print(abs(deltaX[i]))
                # print("yash")
                #print("YASH: ", abs(deltaX[i]))
                if(abs(deltaX[i]) <= 160):
                    #print("ULHAS: ", abs(deltaX[i]))
                    toSendDeltaX = deltaX[i]
                    break
                else:
                    #print("AMBRE: ", abs(deltaX[i]))
                    continue                    
...
def main():
    ...
    try:
        ...
        while True:
            ...
            cv2.imshow("Original Image", frame) # this one
            ...
            if(checkBlue == 1):  
                ...
                if decodedData == "":
                    ...
                    #     pass
                    # else:
                    #     # print("KELA KELA KELA")
                    # # print("show the complete qr code")
                    #     continue
                    print("MAIN: NOT IN THE PID")
                    goForward()
                else:
                    decodedData = str(decodedData)
                    decodedData = decodedData.split("'")
                    ...
                    for i in range(len(motionPath)):
                        ...
                        if (i==0):
                            ...
                            while True:
                                ...
                                cv2.imshow("Follow Image", frame1) # this
                                if cv2.waitKey(1) == ord('c'):
                                    break
